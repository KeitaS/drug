...
def change_flux(cobra_model, gene_list,
    ...
    fold_change: fluxを変更する割合。defoltは0なので、knock out。
    ...
    for the_reaction in the_reactions:
        ...
        if not eval(the_gene_reaction_relation):
            ...
            if wt_reaction_flux > 0:
                the_reaction.upper_bound = wt_reaction_flux * fold_change
            elif wt_reaction_flux < 0:
                the_reaction.lower_bound = wt_reaction_flux * fold_change
            else:
                pass
...
# fluxの値をだす関数
def multi_knockdown(cobra_model, element_list=None,
               method='fba', the_problem='reuse', solver='glpk',
               error_reporting=None, wt_flux=None, fold_change=0.5):# 最後にwt_fluxを追加
    """
    ...
    まだこちらはdoubleに適していない。element_listに二次元リストが追加できない。
    ...
    if isinstance(element_list, list):
        ...
        change_flux(mutant_model, element_list, wt_flux=wt_flux, fold_change=fold_change) # 変異を加える遺伝子
...
if __name__ == "__main__":
    ...
    target_genes = []
    """
    with open("drug_targets.txt", "r") as readfile:
        for lines in readfile.readlines():
            gene_list = re.split(";\s", lines.strip())
            flag = 0
            for gene in gene_list:
                for num in cobra_model.genes:
                    if gene == num:
                        flag += 1
            if flag != 0:
                target_genes.append(gene_list)
    with open("result.tsv", "w") as makecsv:
        for gene_list in target_genes:
            multi_solution = flux_check(wt_model.copy(), gene_list, wt_flux=wt_flux, fold_change=0)[1]
            makecsv.write("%s\t%s\n" % (gene_list, multi_solution.values()[0]))
    """
    target_genes = ["b0084"]
    multi_solution = flux_check(wt_model.copy(), target_genes, wt_flux=wt_flux, fold_change=0)[0]
    with open("mutant.csv", "w") as makecsv:
        result = multi_solution.values()
        # print result
